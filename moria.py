...
def equipe():
    ...
    with col1:
        ...
        st.write('Eduardo Moreira é especialista em Investimentos Quantitativos e Multi-Ativos, com foco em alocação institucional, construção de portfólios globais e estratégias de Fundos de Fundos (FoF). Formado em Economia pela UnB,  com pós-graduação em Estatística e Data Science, atua no ciclo completo de investimentos: desde screening quantitativo, modelagem de fatores e otimização de portfólios até alocação multi-ativos, atribuição de performance e avaliação de riscos institucionais. Na FUNCEF, um dos maiores fundos de pensão do Brasil, liderou o design, desenvolvimento e implementação da primeira estratégia internacional da instituição, incluindo framework de investimentos, due diligence de gestores, seleção de administradores externos e estruturação de veículos exclusivos, com uso de Python, modelos quantitativos, analytics de risco e otimização de portfólio – alcançando forte performance no primeiro ano graças a alocações sistemáticas e processos data-driven. Sua abordagem integra rigor quantitativo e insights qualitativos, com expertise em estratégias globais macro, renda fixa internacional e abordagens sistemáticas multi-ativos, suportada por análises avançadas e engenharia de dados. Palavras-chave: Investimentos Quantitativos, Investimento Sistemático, Fund of Funds, Multi-Ativos, Investimento Institucional, Alocação Global, Construção de Portfólios, Modelagem de Risco, Investimento em Fatores, Pesquisa Quantitativa, Data Science, Estatística, Python para Finanças, Seleção de Gestores, Mercados Globais, Analytics de Investimentos, Otimização de Portfólios.')
        st.markdown('---')
        st.header('Marco Túlio Coutinho')
        st.image('Marco Tulio.png', width= 100)
        st.write('Marco Tulio é  dedicado a capacitar gestores de ativos para excelirem na captação de recursos, fornecendo ferramentas e conhecimentos essenciais para navegarem com confiança pelas complexidades das mais sofisticadas Requests for Proposals (RFPs). Como investidor institucional e consultor de investimentos com 15 anos de experiência no setor, aprimorou habilidades em redação de RFPs, seleção e monitoramento de gestores de ativos, coordenação de due diligence de investimentos e decisões de alocação de ativos. Autor de manuais abrangentes de compliance, estatutos detalhados de comitês de investimento e políticas robustas de relatórios sobre fatores Ambientais, Sociais e de Governança (ESG), além de práticas de Stewardship. Oferece coaching valioso e preparação a General Partners (GPs), garantindo que estejam totalmente equipados para o sucesso em processos de due diligence de investimentos. Para gestores de ativos que buscam prosperar em um cenário competitivo, convida para discutir como pode auxiliar na captação de capital, aprimoramento de respostas a RFPs e elevação das estratégias de investimento a novos patamares de sucesso.')
